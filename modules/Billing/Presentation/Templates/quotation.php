...
$quotationNumber = htmlspecialchars((string) ($data['quotation_number'] ?? ''));
$status = htmlspecialchars((string) ($data['status'] ?? ''));
$createdAt = htmlspecialchars((string) ($data['created_at'] ?? ''));
$expiresAt = htmlspecialchars((string) ($data['expires_at'] ?? '-'));
...
<body>
    <div class="container">
        <div class="header">
            ...
            <div class="doc-meta">
                <h1>Quotation</h1>
                <p><strong># <?= $quotationNumber ?></strong></p>
                <p>Status: <?= $status ?></p>
                <p>Date: <?= $createdAt ?></p>
        ...
        <div class="footer">
            <?php if ($org['name'] ?? ''): ?>
                This quotation is valid until <?= $expiresAt ?>. Please contact <?= htmlspecialchars($org['name']) ?> to accept or discuss.
            <?php else: ?>
                This quotation is valid until <?= $expiresAt ?>.
            <?php endif; ?>
        </div>
    </div>
</body>
