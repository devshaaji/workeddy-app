/**
 * impact-tracker.js — Public Health Impact Tracker filter support
 * Route: GET /reporting/impact-tracker
 *
 * The dashboard itself is server-rendered (observed figures come from real
 * platform activity, estimated figures are derived from disclosed assumption
 * rates — see the page for the methodology note). This script only wires up
 * the filter modal:
...
(function () {
    ...
    var page = document.getElementById('impactTrackerPage');
    if (!page) { return; }
    ...
    function loadOrganizationStructure() {
        ...
        var worksiteSelect = document.getElementById('impactWorksite');
        var departmentSelect = document.getElementById('impactDepartment');
        var jobRoleSelect = document.getElementById('impactJobRole');
    ...
    function loadBodyRegions() {
        var bodyRegionSelect = document.getElementById('impactBodyRegion');
