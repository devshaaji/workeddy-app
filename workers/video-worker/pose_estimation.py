...
def _model_version_label() -> str:
    return f"{Path(_MODEL_PATH).stem}@video-mode"
...
def _risk_label(trunk_angle: float, neck_angle: float, upper_arm_angle: float) -> str:
    if trunk_angle >= 60.0 or upper_arm_angle >= 90.0 or neck_angle >= 30.0:
        return "high"
    if trunk_angle >= 30.0 or upper_arm_angle >= 45.0 or neck_angle >= 20.0:
        return "medium"
    return "low"


def _risky_windows(timeline: list[dict[str, Any]]) -> list[dict[str, Any]]:
    windows: list[dict[str, Any]] = []
    active: dict[str, Any] | None = None

    for point in timeline:
        risk = str(point.get("risk", "low"))
        if risk == "low":
            if active is not None:
                active["end_seconds"] = float(point["time_seconds"])
        ...
        if active is None:
            active = {
                "start_seconds": float(point["time_seconds"]),
                "end_seconds": float(point["time_seconds"]),
                "risk": risk,
            }
            continue
...
def estimate_pose_metrics(
            ...
            while True:
                ...
                if video_w == 0 or video_h == 0:
                    ...
                    if max_resolution is not None:
                        ...
                        scale = min(max_w / max(1, video_w), max_h / max(1, video_h), 1.0)
                        if scale < 1.0:
                            video_w = int(video_w * scale)
                            video_h = int(video_h * scale)

                if max_resolution is not None and (frame.shape[1] != video_w or frame.shape[0] != video_h):
                ...
                trunk_angle = _ema_append(trunk_angles, raw_trunk, smoothing_alpha)
                ...
                neck_angle = _ema_append(neck_angles, neck_flexion, smoothing_alpha)
                ...
                upper_arm_angle = _ema_append(upper_arm_angles, (l_upper + r_upper) / 2.0, smoothing_alpha)

                l_lower = _angle_between_points(_pt(ls), _pt(le), _pt(lw))
                r_lower = _angle_between_points(_pt(rs), _pt(re), _pt(rw))
                lower_arm_angle = _ema_append(lower_arm_angles, (l_lower + r_lower) / 2.0, smoothing_alpha)
                ...
                wrist_angle = _ema_append(wrist_angles, wrist_dev, smoothing_alpha)

                timeline.append(
                    {
                        "time_seconds": round(float(frame_idx / native_fps), 3),
                        "frame": int(frame_idx),
                        "trunk_angle": round(float(trunk_angle), 2),
                        "neck_angle": round(float(neck_angle), 2),
                        "upper_arm_angle": round(float(upper_arm_angle), 2),
                        "lower_arm_angle": round(float(lower_arm_angle), 2),
                        "wrist_angle": round(float(wrist_angle), 2),
                        "risk": _risk_label(float(trunk_angle), float(neck_angle), float(upper_arm_angle)),
                    }
                )
    ...
    if generate_visualization and resolved_output_video_path is not None:
        ...
        thumb_path = str(Path(resolved_output_video_path).with_suffix(".thumb.jpg"))
        thumb_cap = cv2.VideoCapture(resolved_output_video_path)
        ok, thumb_frame = thumb_cap.read()
        thumb_cap.release()
        if ok:
