...
return new class implements SeederInterface
{
    ...
    {
        $existing = $db->fetchAssociative('SELECT * FROM content_pages WHERE page_key = ? LIMIT 1', [NationalImportancePageDefinition::PAGE_KEY]);
        if (is_array($existing)) {
            return;
        }

        $now = gmdate('Y-m-d H:i:s');
        $snapshot = NationalImportancePageDefinition::seedSnapshot();
        $db->insert('content_pages', [
            'uuid' => UuidSupport::generate(),
            'page_key' => NationalImportancePageDefinition::PAGE_KEY,
            'route_path' => '/content/national-importance-context',
            'content_type' => 'managed_page',
            'template_key' => 'national_importance_context',
            'audience' => 'internal',
            'status' => 'draft',
        ...
        $db->insert('content_page_revisions', [
            ...
            'title' => 'Why WorkEddy Matters for Workforce Health',
            'seo_title' => null,
            'seo_description' => null,
            'change_summary' => 'Seeded initial National Importance dashboard context.',
